...
<div class="kt-container  kt-container--fluid  kt-grid_item kt-grid_item--fluid">
    ...
    <!--begin::Form-->
    <div class="card-body">
  <form method="POST"   role="form" action="{{route('admin.managers.update',$manager->id)}}" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <input type="text" class="form-control  @error('name') is-invalid @enderror" name="name" value="{{$manager->name}}"  id="name"  placeholder="Name" required/>
        @error('name')
    ...
    @enderror
      ...
      <div class="form-group">
       ...
       <input type="email" class="form-control  @error('email') is-invalid @enderror" name="email" value="{{$manager->email}}"  placeholder="Enter email" required/>
       @error('email')
       <span class="invalid-feedback" role="alert"  style="display: block">
           <strong>{{ $message }}</strong>
       </span>
   ...
   @enderror
    ...
    <div class="form-group">
        ...
        <input type="url" class="form-control  @error('facebook') is-invalid @enderror" name="facebook" value="{{$manager->facebook}}"  placeholder="FaceBook" />
   ...
   @enderror
       ...
       <div class="form-group">
        ...
        <input type="url" class="form-control @error('twitter') is-invalid @enderror " name="twitter" value="{{$manager->twitter}}"  placeholder="Twitter" />
   ...
   @enderror
       ...
       <div class="form-group">
        ...
        <input type="url" class="form-control @error('github') is-invalid @enderror "  style="display: block" name="github" value="{{$manager->github}}"  placeholder="github" />
        ...
        <div class="col-lg-9 col-xl-6" style="margin-top: 3px">
            <div class="kt-avatar kt-avatar--outline kt-avatar--circle" id="kt_user_avatar_3">
                ...
                <label class="kt-avatar__upload" data-toggle="kt-tooltip" title="" data-original-title="Change avatar">
                    <i class="fa fa-pen"></i>
                    <input type="file" name="image"  value="{{$manager->image}}" accept=".png, .jpg, .jpeg">
                </label>
...
<div class="form-group">
    ...
    <div class="form-group">
      ...
      @foreach ($allpermissions as $permission)
        ...
        <option value="{{$permission->id}}"
             @foreach ($manager->getDirectPermissions() as $permissionmanager)
             @if ($permissionmanager->id==$permission->id)
                 {{'selected'}}
             @endif
    ...
    <div class="form-group">
      ...
      @foreach ($roles as $role)
        ...
        <option value="{{$role->id}}"
            @foreach ($manager->roles as $rolemanager)
            @if ($rolemanager->id==$role->id)
            {{'selected'}}
        @endif
            @endforeach
            >{{$role->name}}</option>
 ...
 <div class="form-group mb-1">
    ...
    <textarea class="form-control @error('description') is-invalid @enderror "  id="exampleTextarea" rows="3" name="description" required>{{$manager->description}}</textarea>
